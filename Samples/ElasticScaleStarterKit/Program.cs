...
namespace ElasticScaleStarterKit
{
    internal class Program
    {
        ...
        private static bool GetMenuChoiceAndExecute()
        {
            while (true)
            {
                ...
                switch (inputValue)
                {
                    ...
                    case 2: // Add shard
                        Console.WriteLine();
                        AddShard(ConsoleUtils.ReadIntegerInput("new Customer Id "));
                        return true;
                    case 3: // Data Dependent Routing
                        Console.WriteLine();
                        int cust = ConsoleUtils.ReadIntegerInput("enter customer ");
                        DataDepdendentRouting(cust);
                        return true;
        ...
        /// </summary>
        private static void DataDepdendentRouting(int customerid)
        {
            ListShardMap<int> shardMap = TryGetShardMap();
            if (shardMap != null)
            {
                DataDependentRoutingSample.ExecuteDataDependentRoutingQuery(
                    shardMap,
                    Configuration.GetCredentialsConnectionString(), customerid);
            }
        }
